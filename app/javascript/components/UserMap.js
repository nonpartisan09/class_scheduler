...
export default class UserMap extends Component{
  ...
  componentDidMount = () => {
    fetch(window.location.origin + '/api/v1/city')
      .then(res => res.json())
      .then(data => {
        this.setState({cities: data.features});
      });
    
    fetch(window.location.origin + '/api/v1/empty')
      .then(res => res.json())
      .then(data => {
      ...
      <div className='userMapContainer'>
        ...
        />
          {this.state.cities.map(cities => {
              ...
              let origin = [cities.geometry.coordinates[0], cities.geometry.coordinates[0]];
