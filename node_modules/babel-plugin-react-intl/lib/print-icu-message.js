...
function getArgumentType(format) {
  const {
    type,
    ordinal
  } = format; // Special-case ordinal plurals to use `selectordinal` instead of `plural`.
...
function printMessageTextASTNode({
  value
}) {
  return value.replace(ESAPE_CHARS_REGEXP, char => ESCAPED_CHARS[char]);
}

function printSimpleFormatASTNode({
  id,
  format
}) {
...
function printOptionalFormatASTNode({
  id,
  format
}) {
  let argumentType = getArgumentType(format);
