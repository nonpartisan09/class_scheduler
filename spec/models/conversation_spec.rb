...
RSpec.describe Conversation, type: :model do
  ...
  describe 'Conversation::check_timely' do
    let(:conversation) { FactoryBot.create(:conversation) }
    
    it 'returns false if volunteer has not responded for more than 48 hours' do
      conversation.messages.create({body: "this is a test", user_id: conversation.author_id, created_at: DateTime.now - 3.days})
      expect(conversation.check_timely).to eq(false)
    end
    ...
    it 'returns true if the message is less than 48 hours old' do
      ...
      expect(conversation.check_timely).to eq(true)
    end
